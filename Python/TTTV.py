...
class TTT(arcade.Window):
    ...
    def __mittelschlauer_zug(self):
        ...
        for y in range(len(self.spielfeld)):
            if (x := self.__kann_gewinnen(self.__zeile(y), self.symbol)) != None:
                return (x, y)
            # Kurzform für:
        ...
        for x in range(len(self.spielfeld)):
            if (y := self.__kann_gewinnen(self.__spalte(x), self.symbol)) != None:
                return (x, y)
        ...
        for y in range(len(self.spielfeld)):
            if (x := self.__kann_gewinnen(self.__zeile(y), "X" if self.symbol == "O" else "O")) != None:
                return (x, y)
            # Kurzform für:
            # x = self.__kann_gewinnen(self.spielfeld[2 - y])
            # if x != None:
        ...
        for x in range(len(self.spielfeld)):
            if (y := self.__kann_gewinnen(self.__spalte(x), "X" if self.symbol == "O" else "O")) != None:
                return (x, y)

        diagonal1, diagonal2 = [self.spielfeld[i][i] for i in range(3)], [self.spielfeld[i][2 - i] for i in range(3)]
        #print(diagonal1, self.__kann_gewinnen(diagonal1), diagonal2, self.__kann_gewinnen(diagonal2))
        if (index := self.__kann_gewinnen(diagonal1), "X" if self.symbol == "O" else "O") != None:
            return (index)
        if (index := self.__kann_gewinnen(diagonal2), "X" if self.symbol == "O" else "O") != None:
