...
def perzeptron(x_werte_sepal_lenght, y_werte, x_werte_sepal_width):
    ...
    w2 = 1.5367513760739622
    w1 = -0.8757524744229066
    b1 =  0.006102963778627439
    a = 0.011
    for j in range(100000):
        for i in range(50):
            ...
            if fehler < 0:
                fehler_w1 = (fehler)**2 * 2 * x1 * -a
                fehler_w2 = (fehler)**2 * 2 * x2 * -a
                fehler_b1 = (fehler)**2 * 2 * -a
            else:
                fehler_w1 = (fehler)**2 * 2 * x1 * a
                fehler_w2 = (fehler)**2 * 2 * x2 * a
                fehler_b1 = (fehler)**2 * 2 * a
