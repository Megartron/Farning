...
class TTT(arcade.Window):
    ...
    def setup(self):
        ...
        self.symbol_spieler = {"O": self.spieler, "X": "ComputerRandom"}
    ...
    def __random_zug(self):
        ...
        # Sonderfall3:
        if ((self.__lesen(0, 1)) and (self.__lesen(1, 2) or self.__lesen(1, 0)) == gegener_symbol ) and self.__lesen(0, 0) == "":
            return (0, 0)
        if ((self.__lesen(2, 1)) and (self.__lesen(1, 2) or self.__lesen(1, 0)) == gegener_symbol ) and self.__lesen(2, 0) == "":
            return (2, 0)
    ...
    def on_mouse_press(self, x: int, y: int, button: int, modifiers: int):
        if self.phase == 0:
            if 200 <= x <= 350 and 72 <= y <= 122:
                self.spieler = "ComputerRandom"
                self.symbol_spieler = {"O": self.spieler, "X": "ComputerRandom"}
                self.phase = 1
            elif 400 <= x <= 550 and 72 <= y <= 122:
                self.spieler = "Mensch"
                self.symbol_spieler = {"O": self.spieler, "X": "ComputerRandom"}
                self.phase = 1            
    ...
    def on_draw(self):
        if self.phase == 0:
            ...
            arcade.draw_text("C vs C", 200, 100, arcade.color.RED_DEVIL, 22, font_name="Kenney Pixel Square", anchor_x="center", anchor_y="center")
            arcade.draw_text("M vs C", 400, 100, arcade.color.RED_DEVIL, 22, font_name="Kenney Pixel Square", anchor_x="center", anchor_y="center")
            arcade.draw_rectangle_outline(400, 97, 150, 50, arcade.color.WHITE, 5)
            arcade.draw_rectangle_outline(200, 97, 150, 50, arcade.color.WHITE, 5)
        elif self.phase == 1:
